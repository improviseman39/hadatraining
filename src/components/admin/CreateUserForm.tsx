...
export default function CreateUserForm() {
  ...
  return (
    <form
      ...
      className="mb-8 flex flex-col gap-3 rounded-2xl border border-ink/10 bg-card p-6 shadow-sm sm:flex-row sm:items-end sm:gap-4"
    >
      ...
      <button
        type="submit"
        disabled={pending}
        className="rounded-full bg-teal px-6 py-2.5 text-sm font-medium text-porcelain transition-colors hover:bg-teal-dark disabled:opacity-60"
      >
        {pending ? "Creating…" : "Create user"}
      </button>

      {error && <p className="text-sm font-medium text-terracotta sm:basis-full">{error}</p>}
      {created && (
        <p className="text-sm font-medium text-teal sm:basis-full">
