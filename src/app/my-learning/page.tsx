...
export default async function MyLearningPage() {
  // Same role gate as /timetable — redirects to /login if signed out, or
  // to / if signed in but not a member (e.g. a "design" role admin).
  const { user } = await requireRole(["user", "admin", "super_admin"]);
  const supabase = await createClient();

  const [{ data: sessionRows }, { data: childParentIds }, { data: progressRow }, { data: sessionProgressRows }] =
    await Promise.all([
      // Only top-level sessions appear on the dashboard grid — nested
      ...
      // Just enough to count sub-topics per top-level session for the badge,
      // without fetching every sub-topic's full row.
  ...
  const continueWatching =
    progressRow && progressSession
      ? {
          sessionSlug: mapSession(progressSession).slug,
          sessionTitle: mapSession(progressSession).title,
          sessionImageUrl: mapSession(progressSession).imageUrl,
  ...
  const overallCompletionPercent =
    sessionProgressRows && sessionProgressRows.length > 0
      ? Math.round(
  ...
  return (
    <div className="container-page py-12 sm:py-16">
      ...
      <p className="mt-3 max-w-xl text-base leading-relaxed text-muted">
        Pick up where you left off, or browse the full curriculum by category.
      </p>
